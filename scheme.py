...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(fn):
            args = Pair(target.second, vals.second)
            exp = Pair('lambda', args)
            env.define(fn, scheme_eval(exp, env))
            return fn
        else:
            raise SchemeError()
    else:
...
def check_formals(formals):
    ...
    for i in range(len(formals)):
        if not scheme_symbolp(formals[i]):
            raise SchemeError(str('symbol {0} is not valid'.format(formals[i])))
        if formals[i] in mydict:
            mydict[formals[i]] += 1
        else:
            mydict[formals[i]] = 1
    for value in mydict.values():
        if value > 1:
            raise SchemeError("expected distinct formal parameters")

    # TODO, also can check using a list, if already in the list, then raise error, else add to list
